...
class Cott(commands.Cog):
    ...
    async def dolar(self, ctx, quantia=1.00):
        ...
        embed.add_field(name='Valor em Dólares Americanos (USD)', value=f"U${float(quantia):.2f}", inline=False)
        embed.add_field(name='Valor em Reais Brasileiros (BRL)', value=f"R${valor*float(quantia):.2f}", inline=False)
